...
#' #' @example inst/examples/examples.R
#' #' @import ggplot2
#' plotPolytope<-function(cPoints = NULL, points = NULL, rangePoints = NULL, showLbl=FALSE, iso=NULL, crit="max", latex = FALSE,
#'    feasible = rbind(points[,c("x1","x2","lbl")], cPoints[,c("x1","x2","lbl")], rangePoints[,c("x1","x2","lbl")]), ...)
#' {
#'    if (is.null(points) & is.null(rangePoints) & is.null(cPoints)) stop("Arguments cPoints, points or rangePoints must be specified!")
#'    # Set Custom theme
#'    myTheme <- theme_bw()
