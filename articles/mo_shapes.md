...
We define the sets $`\mathbb{R}_{>} = \{ z\in \mathbb{R} \mid z > 0\}`$,
$`\mathbb{R}_{\geqq} = \{ z\in \mathbb{R} \mid z \geqq 0\}`$ and
$`\mathbb{R}_{\geqslant} = \{ z\in \mathbb{R} \mid z \geqslant 0\}`$:
...
$`U`$ with local upper bounds $`N(U)`$, the search region $`A`$ is given
by the closure of the gray area:
...
in the objective space by applying objective branching on $`y1`$, $`y2`$
and $`y3`$, represented by the red circles. The constraints added when
